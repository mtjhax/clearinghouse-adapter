...
require_relative '../../processors/advanced_processors/advanced_import_processor'
...
describe ImportProcessor do
  ...
  #
  # TEST THE CUSTOM MAPPING FEATURES OF THE ADVANCED IMPORT PROCESSOR
  #
  describe 'advanced import functionality' do
    describe "#process" do
      before do
        @sample_file_base = 'processors/advanced_processors/sample_data'
        @sample_trip_import = File.join(@sample_file_base, '1 - trip import.csv')
        @sample_trip_result_import = File.join(@sample_file_base, '5 - trip result import.csv')
      end

      it 'imports trips and maps input attributes' do
        FileUtils.cp @sample_trip_import, @input_folder
        results = @import_processor.process
        results.size.must_equal 2
        expected_result = {"origin_trip_id"=>"789123", "status"=>"active", "origin_customer_id"=>"89456", "customer_first_name"=>"Samuel", "customer_last_name"=>"Clemens", "customer_middle_name"=>"L", "customer_dob"=>"1935-11-30", "customer_primary_phone"=>"(503) 123-4567", "customer_emergency_phone"=>"(503) 234-5678", "customer_address_attributes"=>{"address_type"=>"Customer Home", "phone_number"=>"(503) 123-4567", "common_name"=>"Rosemont Court", "address_1"=>"597 N Dekum St #333", "city"=>"Portland", "state"=>"OR", "zip"=>"97217"}, "customer_primary_language"=>"English", "customer_gender"=>"m", "customer_internal_id"=>"X22356M", "customer_notes"=>"This is a customer note", "customer_boarding_time"=>"5", "customer_deboarding_time"=>"5", "customer_seats_required"=>"1", "customer_service_level"=>"Wheelchair", "customer_mobility_factors"=>["Lift Needed"], "customer_eligibility_factors"=>["STF"], "num_attendants"=>"0", "num_guests"=>"0", "pick_up_location_attributes"=>{"phone_number"=>"(503) 123-4567", "common_name"=>"Rosemont Court", "address_1"=>"597 N Dekum St #333", "city"=>"Portland", "state"=>"OR", "zip"=>"97217"}, "drop_off_location_attributes"=>{"address_type"=>"Business", "phone_number"=>"(503) 528-1747", "common_name"=>"Ride Connection", "address_1"=>"9955 NE Glisan St", "city"=>"Portland", "state"=>"OR", "zip"=>"97220"}, "requested_pickup_time"=>"2014-11-01T07:00-0800", "requested_drop_off_time"=>"2014-11-01T07:15-0800", "appointment_time"=>"2014-11-01T07:15-0800", "time_window_before"=>"10", "time_window_after"=>"10", "scheduling_priority"=>"dropoff", "trip_purpose_description"=>"Personal Business", "trip_funders"=>["STF"], "estimated_distance"=>"5.52", "trip_notes"=>"This is a trip note"}
        expected_result.each do |attr, value|
          results[0][attr].must_equal(value)
        end
      end

      it 'imports trips results and maps inputs to a nested trip result in the output' do
        FileUtils.cp @sample_trip_result_import, @input_folder
        results = @import_processor.process
        results.size.must_equal 2
        expected_result = {"ch_id"=>"12345", "origin_trip_id"=>"23456", "trip_result_attributes"=>{"outcome"=>"completed", "actual_pick_up_time"=>"2014-11-01T07:04-0800", "actual_drop_off_time"=>"2014-11-01T7:14-0800", "fare"=>"0", "odometer_start"=>"45682", "odometer_end"=>"45690", "driver_id"=>"Moore, Mary Tyler", "vehicle_id"=>"Bus 1"}}
        expected_result.each do |attr, value|
          results[0][attr].must_equal(value)
        end
        expected_result = {"ch_id"=>"12346", "origin_trip_id"=>"23457", "trip_result_attributes"=>{"outcome"=>"no-show"}}
        expected_result.each do |attr, value|
          results[1][attr].must_equal(value)
        end
      end
    end
  end
  ...
  #
  # TESTS EVERYTHING THE BASIC IMPORT PROCESSOR TESTED TO MAKE SURE IT STILL WORKS WITH ADVANCED PROCESSOR
